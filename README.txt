# javascript-practice
This is a brief log of the concepts I'm encountering as I learn.
...
Language Basics:
	...
	variables are loosely typed
		attention should be paid to typing - don't type change
	local variable if var is used in declaration
	global variable if var is omitted
		not recommended, not allowed in strict mode
	5 data types:
	...
	no overloading in JS
		the last definition of the function that becomes added to the interpreter environment
...
CSS:
	...
	Codewars:
		.map() (translate each element in an array into something else, in place)
		.reduce() (gets called on every element of array) .reduce(function(s,e) {return s+e;})
...
k		delete <object.prop>
		...
		<arr>.filter(<callback>);
		arguments to ACTUAL array (so you can use array functions on it):
